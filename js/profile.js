...
async function renderProfile(userData) {
    ...
    // Daily digest with lucky number
    if (DOM.dailyDigest) {
    ...
    // Hide name section if already set
...
async function renderLeaderboard(period = 'all', category = null) {
    ...
    : data.map((u, i) => {
        ...
        return `<div style="display:flex;justify-content:space-between;padding:8px;" class="${u.positionChange !== 0 ? 'leaderboard-glow' : ''}"><div>${i + 1}. ${u.avatar || ''} ${uTitle.emoji} ${escapeHtml(name)}${changeIcon}</div><div>👁️ ${u.seerscore}</div></div>`;
    }).join('');
    ...
    try {
        ...
        if (seer && DOM.seerOfTheDay) {
            ...
            DOM.seerOfTheDay.innerHTML = `<div style="text-align:center;"><div style="font-size:2rem;">🌟</div><div>👁️ Seer of the Day</div><div style="font-size:1.2rem;color:var(--accent);">${seerTitle.emoji} ${seer.display_name || (seer.address || '').slice(0, 6)}...</div><div style="font-size:.8rem;">${seerTitle.title} • SeerScore: ${seer.seerscore || '???'}</div></div>`;
        }
    } catch (err) { console.error('Seer of day error:', err); }
}
