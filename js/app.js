// ============================================================
// PRAEDICTA – Prediction Market App.js (v8 – Resolution System)
// ============================================================
...
function buySharesMock(id, outcome, amount) { const market = getMarket(id); const price = outcome === 'yes' ? getYesPrice(market.yesPool, market.noPool) : 1 - getYesPrice(market.yesPool, market.noPool); const cost = amount * price; if (userPRAEBalance < cost) return { error: 'Insufficient PRAE balance.' }; userPRAEBalance -= cost; saveBalance(); if (outcome === 'yes') { market.yesPool += amount; market.noPool = (market.yesPool * market.noPool) / market.yesPool; } else { market.noPool += amount; market.yesPool = (market.yesPool * market.noPool) / market.noPool; } return { cost, shares: amount }; }
...
function renderPredictionCard(p, isOracle) {
    const active = p.status === 'active'; const market = getMarket(p.id); const yesPrice = getYesPrice(market.yesPool, market.noPool); const noPrice = 1 - yesPrice;
    const deadline = p.resolution_date ? formatDateWithoutSeconds(p.resolution_date) : 'No deadline'; const canResolve = p.status === 'expired' && isOracle;
    ...
    const catIcon = CATEGORY_ICONS[p.category] || '📁'; const displayCat = p.category === 'crypto' ? 'Finance' : p.category;
...
async function buyClick(e) {
    ...
    if (amount < CONFIG.MIN_BET) return showToast(`Minimum bet is ${CONFIG.MIN_BET} PRAE.`); if (!['yes', 'no'].includes(outcome)) return;
    ...
    try { const result = useRealMarket ? await buySharesReal(id, outcome, amount) : buySharesMock(id, outcome, amount); if (result.error) { showToast(result.error); } else { showToast(`Bought ${result.shares.toFixed(2)} ${outcome.toUpperCase()} shares for ${result.cost.toFixed(2)} PRAE.`); await refreshAll(); } } catch (err) { console.error('Buy error:', err); showToast('Transaction failed.'); } finally { setLoading(btn, false); actionCooldown = false; }
}
...
async function reactClick(e) {
    ...
    try { await callSecureRpc('react', { predictionId: id, reaction: emoji }); } catch (err) { console.error('Reaction save failed:', err); }
...
async function resolveClick(e) {
    ...
    if (outcome === 'unresolvable') { if (!confirm("Mark as UNRESOLVABLE? All bets refunded, creator loses SeerScore.")) return; }
    else { if (!confirm(`Resolve to ${outcome.toUpperCase()}? Winners paid from losing pool.`)) return; }
    try { if (outcome === 'unresolvable') { await callSecureRpc('unresolvable', { predictionId: id }); } else { await callSecureRpc('resolve', { predictionId: id, outcome }); } showToast(outcome === 'unresolvable' ? "Marked unresolvable - all bets refunded" : `Resolved ${outcome.toUpperCase()} - payouts calculated!`); await refreshAll(); } catch (err) { console.error('Resolve error:', err); showToast('Failed to resolve.'); }
}

// ── Data Loading ───────────────────────────────────────────
async function loadPredictions() { try { const { data, error } = await supabaseClient.from('predictions').select('*').order('created_at', { ascending: false }).limit(100); if (error) throw error; return data || []; } catch (err) { console.error('Load predictions error:', err); return []; } }
...
async function renderProfile(userData) {
    ...
    if (DOM.profileStats) DOM.profileStats.innerHTML = `<div><span style="color:var(--accent);">💰 PRAE Balance:</span> ${userPRAEBalance.toFixed(2)}</div><div><span style="color:var(--accent);">👁️ Seerscore:</span> ${seerScore}</div><div style="margin-top:8px;"><span style="color:var(--oracle-color);">${prophetTitle.emoji} Rank:</span> ${prophetTitle.title}</div>`;
    if (DOM.dailyDigest) { const horo = user.zodiac ? await fetchHoroscopeForZodiac(user.zodiac) : null; DOM.dailyDigest.innerHTML = '📊 Yesterday: you traded wisely.'; if (horo?.luckyNumber) DOM.dailyDigest.innerHTML += `<br>🍀 Lucky Number: <strong>${horo.luckyNumber}</strong>`; }
...
async function refreshAll() {
    try { const predictions = await loadPredictions(); currentPredictions = predictions; renderPraedictions(); const user = await loadUser(walletAddress); await renderProfile(user); const activeTab = document.querySelector('.tab-content.active'); if (activeTab?.id === 'tab-leaderboard') await renderLeaderboard(leaderboardPeriod, DOM.leaderboardCategoryFilter?.value || null); const activeCount = predictions.filter(p => p.status === 'active').length; if (DOM.totalActive) DOM.totalActive.textContent = activeCount; const totalBet = Object.values(mockMarkets).reduce((s, m) => s + m.yesPool + m.noPool - (MOCK_INITIAL_POOL * 2), 0); if (DOM.totalPraedicts) DOM.totalPraedicts.textContent = totalBet.toFixed(0); if (DOM.crowdYes) DOM.crowdYes.textContent = '50%'; const totalVolume = Object.values(mockMarkets).reduce((s, m) => s + Math.abs(m.yesPool - MOCK_INITIAL_POOL) + Math.abs(m.noPool - MOCK_INITIAL_POOL), 0); if (DOM.totalVolume) DOM.totalVolume.textContent = totalVolume.toFixed(0); if (user && DOM.totalSeerscore) DOM.totalSeerscore.textContent = user.seerscore || 0; const hottest = getHottestCategory(); if (DOM.hottestCategory) DOM.hottestCategory.innerHTML = `${hottest.icon} Hottest: <strong>${hottest.name}</strong> (${hottest.count} active)`; const isOracle = (walletAddress === CONFIG.ORACLE_WALLET); if (DOM.oracleIndicatorTop) DOM.oracleIndicatorTop.style.display = isOracle ? 'inline-flex' : 'none'; if (DOM.oracleIndicatorProfile) DOM.oracleIndicatorProfile.style.display = isOracle ? 'inline-block' : 'none'; updateHypeMessage(); } catch (err) { console.error('Refresh error:', err); }
}

// ── Connection & Session ───────────────────────────────────
async function callSecureRpc(action, params = {}) { if (sessionToken && sessionExpiresAt > Date.now()) { try { const res = await fetch(CONFIG.SECURE_RPC_URL, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ action:'session_action', params:{ action, params }, token:sessionToken }) }); if (res.ok) return await res.json(); sessionToken = null; sessionExpiresAt = null; } catch (e) {} } if (!walletAddress) throw new Error("Wallet not connected"); const { data: nonce, error: nonceErr } = await supabaseClient.rpc('get_auth_nonce', { p_wallet: walletAddress }); if (nonceErr) throw new Error("Nonce error: " + nonceErr.message); const message = `Login to PRAEDICTA at praedictacoin.github.io`; const encoded = new TextEncoder().encode(message); const signed = await window.solana.signMessage(encoded); const signature = Array.from(signed.signature).map(b => b.toString(16).padStart(2, '0')).join(''); const res = await fetch(CONFIG.SECURE_RPC_URL, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ action, params, signature, wallet: walletAddress, nonce, message }) }); if (!res.ok) { const err = await res.json().catch(() => ({})); throw new Error(err.error || 'Secure RPC failed'); } return await res.json(); }
...
async function connectWallet() {
    if (!DOM.ageCheckbox.checked) return showToast("Confirm age and Terms"); if (!window.solana) return alert("Install Phantom wallet to continue.");
...
function initEventListeners() {
    DOM.themeToggleSmall?.addEventListener('click', toggleTheme); DOM.connectBtn?.addEventListener('click', connectWallet); DOM.disconnectBtn?.addEventListener('click', disconnectWallet);
    ...
    DOM.filterCategory?.addEventListener('change', e => { currentFilter.category = e.target.value; renderPraedictions(); });
    ...
    document.querySelectorAll('.category-carousel-btn').forEach(btn => { btn.addEventListener('click', function() { document.querySelectorAll('.category-carousel-btn').forEach(b => b.classList.remove('active-filter')); this.classList.add('active-filter'); currentFilter.category = this.dataset.category; renderPraedictions(); }); });
    document.querySelectorAll('.status-filter-btn[data-status]').forEach(btn => { btn.addEventListener('click', function() { document.querySelectorAll('.status-filter-btn[data-status]').forEach(b => b.classList.remove('active-filter')); this.classList.add('active-filter'); currentFilter.status = this.dataset.status; renderPraedictions(); }); });
    ...
    DOM.showMyPraedictionsBtn?.addEventListener('click', () => { const c = DOM.myPraedictionsList; if (!c) return; if (c.style.display === 'none' || !c.style.display) { const myCreated = currentPredictions.filter(p => p.creator === walletAddress); const myBets = currentPredictions.filter(p => (p.bets || []).some(b => b.user === walletAddress) && p.creator !== walletAddress); let html = ''; if (myCreated.length > 0) { html += '<h4 style="color:var(--accent);margin-bottom:8px;">✨ Created by you</h4>'; html += myCreated.map(p => `<div style="background:var(--card-bg);border-radius:12px;padding:12px;margin-bottom:8px;"><strong>${escapeHtml(p.title)}</strong><br><span style="font-size:.8rem;color:var(--text-muted);">Status: ${p.status.toUpperCase()}</span></div>`).join(''); } if (myBets.length > 0) { html += '<h4 style="color:var(--accent);margin:12px 0 8px;">💰 Your bets</h4>'; html += myBets.map(p => { const bet = p.bets.find(b => b.user === walletAddress); return `<div style="background:var(--card-bg);border-radius:12px;padding:12px;margin-bottom:8px;"><strong>${escapeHtml(p.title)}</strong><br><span style="font-size:.8rem;color:var(--text-muted);">${bet.outcome.toUpperCase()} • ${p.status}</span></div>`; }).join(''); } c.innerHTML = html || 'No praedictions yet.'; c.style.display = 'block'; } else { c.style.display = 'none'; } });
...
// ── Initialization ─────────────────────────────────────────
function init() { initTheme(); initEventListeners(); if ('serviceWorker' in navigator) { window.addEventListener('load', () => { navigator.serviceWorker.register('/test/sw.js').then(reg => console.log('SW registered:', reg.scope)).catch(err => console.error('SW failed:', err)); }); } if (window.solana?.isConnected) setTimeout(() => DOM.connectBtn?.click(), 100); setInterval(updateCountdowns, 60000); }
init();
