...
async function callSecureRpc(action, params = {}) {
    // Route email actions to email_verify edge function
    ...
    // Route admin actions to admin edge function
    if (['admin_get_stats', 'admin_action'].includes(action)) {
        const res = await fetch(`${CONFIG.SUPABASE_URL}/functions/v1/admin`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: action === 'admin_get_stats' ? 'get_stats' : params.action, wallet: walletAddress, params: params.params || params })
        });
        return await res.json();
    }

    // Route order book actions to order edge function
    if (['place_order', 'cancel_order', 'get_orderbook', 'get_user_orders'].includes(action)) {
        const res = await fetch(`${CONFIG.SUPABASE_URL}/functions/v1/place_order`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action, params: { ...params, wallet: walletAddress } })
        });
        return await res.json();
    }
...
async function connectWallet() {
    ...
    try {
        ...
        if (params.get('ref')) { showToast("🎁 You were referred! Both earn rewards.", 'success'); }
...
async function createPrediction() {
    ...
    try {
        ...
        creatorBetOutcome = null; if (DOM.creatorBetYes) DOM.creatorBetYes.classList.remove('selected'); if (DOM.creatorBetNo) DOM.creatorBetNo.classList.remove('selected'); if (DOM.creatorBetDisplay) { DOM.creatorBetDisplay.textContent = 'Select your conviction'; DOM.creatorBetDisplay.style.color = 'var(--text-muted)'; }
        await refreshAll();
